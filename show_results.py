...
nb_captors = [i.split("_")[2] for i in list(os.listdir('./center_and_reduce'))]
nb_captor = st.selectbox('Choose the number of captor', nb_captors) # Créer un sélecteur pour choisir le modèle à afficher
...
if "final_results.pkl" in list(os.listdir(f"./center_and_reduce/nb_captor_{nb_captor}")):
...
final_results_captor_models = final_resultats.set_index(["Captor", 'Model_Name'], inplace=False)
final_results_groupby_captor = final_resultats.groupby("Model_Name").describe()
c1_captor_models, c2_group_by_models = st.columns(2)
metrics = list(
    final_results_groupby_captor.columns.get_level_values(0).unique()
)
with c1_captor_models:
    st.header("Metrics for each captors on every models")
    st.dataframe(final_results_captor_models)
with c2_group_by_models:
    st.header("Summary metrics for each models on all captors")
    metric = st.selectbox('Select the metric', metrics) # Créer un sélecteur pour choisir le modèle à afficher
    TGCN_model, LSTM_model, GRU_model = st.columns(3)
    with TGCN_model:
        st.header("TGCN")
        st.dataframe(final_results_groupby_captor.loc["TGCN Model"].loc[metric])
    with LSTM_model:
        st.header("LSTM")
        st.dataframe(final_results_groupby_captor.loc["LSTM Model"].loc[metric])
    with GRU_model:
        st.header("GRU")
        st.dataframe(final_results_groupby_captor.loc["GRU Model"].loc[metric])
...
# ratio = final_resultats.div(final_resultats.iloc[0])
...
# for captor_id in final_resultats.index.get_level_values("Captor").unique():
#     best_model_name = final_resultats.loc[captor_id]["RMSE"].idxmin()
#     best_model_result = final_resultats.loc[captor_id].loc[best_model_name]["RMSE"]
#     stats[best_model_name]["nb_win"] += 1
#     good_results[best_model_name].append(best_model_result)
#     for model_name in model_names:
#         if model_name != best_model_name:
#             bad_result = final_resultats.loc[captor_id].loc[model_name]["RMSE"]
